...
int env_set_key(char *key, char *value, ProgramData *program_data)
{
	int i, key_length = 0, is_new_key = 1;

	/* validate the arguments */
	if (key == NULL || value == NULL || program_data->env == NULL)
		return (1);

	/* obtains the leng of the variable requested */
	key_length = string_length(key);

	for (i = 0; program_data->env[i]; i++)
	{/* Iterates through the environ and check for coincidence of the vame */
		if (string_compare(key, program_data->env[i], key_length) &&
		program_data->env[i][key_length] == '=')
		{/* If key already exists */
			...
			free(program_data->env[i]);
			break;
		}
	}
	/* make an string of the form key=value */
	program_data->env[i] = string_concatenate(string_duplicate(key), "=");
	program_data->env[i] = string_concatenate(program_data->env[i], value);

	if (is_new_key)
	{/* if the variable is new, it is create at end of actual list and we need*/
	/* to put the NULL value in the next position */
		program_data->env[i + 1] = NULL;
	}
	return (0);
}
/**
...
int env_remove_key(char *key, ProgramData *program_data)
{
	...
	{/* iterates through the environ and checks for coincidences */
		...
		{/* if key already exists, remove them */
			...
			i++;
			for (; program_data->env[i]; i++)
			{
				program_data->env[i - 1] = program_data->env[i];
			}
...
void print_environ(ProgramData *program_data)
{
	...
	for (j = 0; program_data->env[j]; j++)
	{
		print_to_stdout(program_data->env[j]);
		print_to_stdout("\n");
	}
}
